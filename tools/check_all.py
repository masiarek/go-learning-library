...
GATES: list[tuple[str, list[str]]] = [
    ...
    # gofmt -l lists unformatted files and still exits 0, so the list itself is the verdict.
    ("gofmt", [sys.executable, "-c",
               "import subprocess, sys; out = subprocess.run(['gofmt', '-l', '.'], capture_output=True, text=True).stdout; print(out, end=''); sys.exit(1 if out.strip() else 0)"]),
